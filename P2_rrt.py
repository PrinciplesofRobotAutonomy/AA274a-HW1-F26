...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False, plot=True):
        """
        ...
        Inputs:
            ...
            shortcut: whether to call shortcut_path on the solution (and, if
                plot is True, also plot the path before and after shortcutting)
            plot: whether to plot the tree and solution (set False for
                batch experiments)
        Output:
            Boolean, True if a solution was found. See also the
            "Intermediate Outputs" descriptions below
        """
        ...
        #    - self.num_iters: the number of RRT iterations (loop passes, including ones
        #          whose extension was rejected by collision checking) that were run before
        #          returning, i.e. the iteration at which the goal was reached, or max_iters
